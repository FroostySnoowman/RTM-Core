...
public class GamemodeSurvival implements TabExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
        ...
        if (sender instanceof Player) {
            if (sender.hasPermission("rtm.gms")) {
                Player player = (Player) sender;
                String type = "survival";
                try {
                    Player target = Bukkit.getPlayer(args[0]);
                    if (sender.hasPermission("rtm.gms.other")) {
                        if (target != null) {
                        ...
                        sender.sendMessage(MiniMessage.miniMessage().deserialize(prefix_message + ' ' + no_permission_message, Placeholder.component("permission", Component.text("rtm.gms.other"))));
                    }
                } catch (Exception e) {
                ...
                sender.sendMessage(MiniMessage.miniMessage().deserialize(prefix_message + ' ' + no_permission_message, Placeholder.component("permission", Component.text("rtm.gms"))));
            }
        } else {
